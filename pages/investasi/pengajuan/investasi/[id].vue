    ...
    <div class="content">
        ...
        <el-card class="!rounded-2xl !mb-6" shadow="never">
            <el-row :gutter="20">
                ...
                <el-col :md="6">
                    <div class="float-end">
                        <img :src="data.umkm_logo" alt="umkm-logo" class="rounded-full border max-w-48">
                    </div>
                </el-col>
            </el-row>
            <base-reader label="Alamat" class="mb-4">
                {{ data.alamat }}
            </base-reader>
            <base-reader label="Kartu Tanda Penduduk (KTP)" class="mb-4">
            ...
            <el-row :gutter="20" class="mb-4">
                <el-col :md="12">
                    <base-reader label="Jumlah Pendanaan yang Dibutuhkan">
                        {{ data.investasi.pendanaan }}
                    </base-reader>
                </el-col>
                <el-col :md="12">
                    <base-reader label="Jangka Waktu Funding">
                        {{ data.investasi.tgl_mulai }} - {{ data.investasi.tgl_selesai}}
                    </base-reader>
                </el-col>
            </el-row>
            <base-reader label="Pembagian Keuntungan" class="mb-4">
            {{ data.investasi.deviden_pemilik }} Pemilik Usaha : {{ data.investasi.deviden_investor}}
            </base-reader>
            <el-row :gutter="20" class="mb-4">
                <el-col :md="12">
                    <base-reader label="Jangka Waktu Investasi">
                        {{ data.investasi.tempo }} <span class="text-sm text-zinc-400">(Terhitung saat pengajuan disetujui)</span>
                    </base-reader>
                </el-col>
                <el-col :md="12">
                    <base-reader label="Periode Pembagian Bunga/Dividen">
                        {{ data.investasi.deviden_periode }}
                    </base-reader>
                </el-col>
            </el-row>
            <base-reader label="Deskripsi" class="mb-4">
            {{ data.investasi.deskripsi }}
            </base-reader>
            <base-reader label="Dokumen Rencana Keuangan" class="mb-4">
                <a href="">{{ data.investasi.dok_rencana_keuangan }}</a>
            </base-reader>
            <base-reader label="Dokumen Analisis Risiko Usaha" class="mb-4">
                <a href="">{{ data.investasi.dok_analisis_resiko }}</a>
            </base-reader>
            <base-reader label="Dokumen ROI (Return on Investment)" class="mb-4">
                <a href="">{{ data.investasi.dok_roi }}</a>
            </base-reader>
            <base-reader label="Dokumen Rencana Bisnis" class="mb-4">
                <a href="">{{ data.investasi.dok_rencana_bisnis }}</a>
            </base-reader>
            <base-reader label="Dokumen Kerja Sama" class="mb-4">
                <a href="">{{ data.investasi.dok_perjanjian }}</a>
            </base-reader>
            <div class="flex justify-end">
                <base-reader label="Tanda Tangan Digital">
                    <img :src="data.investasi.ttd" class="h-full"/>
                </base-reader>
            </div>
        </el-card>
        <el-dialog v-model="openModal" title="" class="!rounded-3xl" width="500">
            ...
            <div class="p-3 text-center" v-else>
                ...
                <h3 class="font-semibold text-xl">Tolak Pengajuan Investasi</h3>
                <p class="text-base font-normal leading-6 text-zinc-400">Anda yakin ingin menolak pengajuan investasi ini? Keputusan ini tidak dapat dibatalkan, karena perubahan tidak bisa dilakukan setelahnya</p>
                
                <div class="flex justify-center">
                    <el-button @click.prevent="cancelAction" class="!rounded-3xl !px-5 !py-3 !h-10">
                        Review Kembali
                    </el-button>
                    <el-button type="danger" @click.prevent="openAction('setuju')" class="!rounded-3xl !px-5 !py-3 !h-10">
                        Ya, tolak pengajuan
                    </el-button>
                </div>
            </div>
...
useHead({ 
    title: 'Konfirmasi Pengajuan Investasi' 
});
